...
class AnimatedCard extends StatefulWidget {
  ...
  const AnimatedCard({required this.index, required this.onAdd, Key? key})
    : super(key: key);
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [
            if (isExpanded) BoxShadow(color: Colors.black26, blurRadius: 8),
          ],
        ),
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 8),
            Text(
              "Pacote de Viagem ${widget.index + 1}",
              style: TextStyle(fontWeight: FontWeight.bold),
            ),
            Text("Explore o mundo!", style: TextStyle(color: Colors.black54)),
            if (isExpanded) ...[
              SizedBox(height: 12),
              CustomPaint(
                size: Size(60, 60),
                painter: MeterPainter(
                  progress: meterController.value * meterValue,
                ),
              ),
              SizedBox(height: 12),
